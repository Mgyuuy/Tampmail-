...
def generate_gmail_dot_variations(gmail: str, count: int = 50):
    ...
    if domain != 'gmail.com':
        return ["This bot works only with Gmail addresses.@rajaraj01"]
...
def generate_gmail_plus_variations(gmail: str, count: int = 50):
    ...
    if domain != 'gmail.com':
        return ["This bot works only with Gmail addresses.@rajaraj_01"]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Welcome! Please enter your Gmail address.@rajaraj_01"
    )
...
def main():
    token = "7836397517:AAEkiP_J2AFXshq0wwMyV421R09booFGFYM"  # Replace with your bot token
